...
class MyTestCase1(unittest.TestCase):
    ...
    # % _
        ...
        #print(randomCAK)
        for i in range(0,1):
            l='abcd'+""  #random.choice(string.punctuation) can be used to get a random special character
            response = self.app.get(BASE_URL+'?Calling_API_Key='+urllib.parse.quote(l.encode('utf8'))+'&Page=1&Page_Limit=10',
                                    headers = {'Authorization': 'Basic ' + base64.b64encode(bytes(appp.userN+':'+appp.passW,'ascii')).decode('ascii')})
            data = json.loads(response.get_data())
            pprint.pprint(l)
            pprint.pprint(urllib.parse.quote(l))
            pprint.pprint(urllib.parse.quote(l.encode('utf8')))
            pprint.pprint(data)
            ...
            for i in range(0,len(data[1]['tasks'])):
                self.assertTrue(l.lower() in (data[1]['tasks'][i]['CallingAPIKey']).lower(),
